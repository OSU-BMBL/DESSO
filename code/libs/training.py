''' The module used in model training '''
from __future__ import division
import sys
import time
import numpy as np
from encode_cons import *
...
def train(train_data, train_targets, validation_data, validation_targets, path_curr_net, net, drop_out_rate, learning_rate_init, learning_momentum, lambda_regulate, std_weights, test_data = None, test_targets = None):
    '''
    Args:
        train_data: 'Numpy.ndarray' representing training data;
        train_targets: 'Numpy.ndarray' representing targets/lables of the above training data;
    ...
    with tf.Graph().as_default():
        ...
        #conv1_weights, conv1_biases = util.para_trainable([filter_length, train_data.shape[2], num_channels, filter_num], std_weights, 0, 'conv1')
        conv1_weights = {}
        conv1_biases = {}
        ...
        # Prediction of each minibatch training data
        ...
        saver = tf.train.Saver()    # Used to save the model with maximum AUC
        start_time = time.time()    # Used to record the time of each parameter update
            ...
            while epoch < NUM_EPOCHS and (not done_looping):
                ...
                for start_point in xrange(0, train_size - BATCH_SIZE, BATCH_SIZE):
                    ...
                    if (iteration_count + 1) % eval_frequency == 0:
                        ...
                        validation_AUC = util.calc_auc(eval_in_batches(validation_data, sess).flatten(), validation_targets)
                        ...
                        if validation_AUC > best_validation_perf:
                            ...
                            if test_data is not None:
                                test_AUC = util.calc_auc(eval_in_batches(test_data, sess).flatten(), test_targets)
